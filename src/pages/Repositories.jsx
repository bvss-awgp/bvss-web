...
const Repositories = () => {
  ...
  const handleCategoryChange = async (repoId, newCategory) => {
    setUpdatingCategory((prev) => ({ ...prev, [repoId]: true }));
    setEditingCategoryId(null);

    try {
      const response = await fetch(getApiUrl(`/admin/repositories/${repoId}/category`), {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      setRepositories((prev) =>
        prev.map((repo) =>
          repo._id === repoId ? { ...repo, category: newCategory } : repo
        )
      );
    } catch (error) {
                ...
                >
                  {isLoadingRepositories ? (
                    <>
                      ...
                      <span className="hidden sm:inline">{t("admin.refreshing") || "Refreshing..."}</span>
                    </>
                  ) : (
                    <>
                      <svg
                            ...
                            {filteredRepositories.map((repo, index) => {
                              ...
                              return (
                                <tr key={repo._id} className="hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200">
                                  ...
                                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                                    {editingCategoryId === repo._id ? (
                                      <select
                                        defaultValue={repo.category}
                                        onChange={(e) => handleCategoryChange(repo._id, e.target.value)}
                                        onBlur={() => setEditingCategoryId(null)}
                                        autoFocus
                                        className="px-3 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800 border-2 border-indigo-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none cursor-pointer"
                                      >
                                        {researchCategoryOptions.map((option) => (
                                          <option key={option.value} value={option.value}>
                                            {option.label}
                                      ...
                                      <button
                                        onClick={() => setEditingCategoryId(repo._id)}
                                        disabled={updatingCategory[repo._id]}
                                        className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800 hover:bg-indigo-200 transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed group"
                                        title="Click to edit category"
                                      >
                                        {updatingCategory[repo._id] ? (
                                          <>
                                          ...
                                          <>
                                            <span>{getCategoryLabel(repo.category)}</span>
