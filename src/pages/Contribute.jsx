...
const Contribute = () => {
  ...
  const createEmptyForm = useCallback(
    () => {
      const nameParts = user?.name ? splitName(user.name) : { firstName: "", lastName: "" };
      return {
    ...
    [user?.email, user?.name, splitName]
  );
  ...
  // Update form data when user object changes (e.g., after signup) - only if form is empty
  useEffect(() => {
    if (!hasExistingContribution && !isLoadingProfile && user && user.name) {
      // Only pre-fill if firstName and lastName are both empty
  ...
  }, [user?.name, user?.email, hasExistingContribution, isLoadingProfile]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    const loadContribution = async () => {
      try {
        ...
        if (!payload?.contribution) {
          // Pre-fill form with user name if available
